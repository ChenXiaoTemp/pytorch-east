...
parser.add_argument('--img_path', dest='training_images_path', action='store',
                    default="{}/ch4_training_images".format(LOCAL_WORKSPACE_HOME),
...
parser.add_argument('--max-text-size', dest='max_text_size', action='store',
                    default=800,
                    help='if the text in the input image is bigger than this, then we resize'
                         'the image according to this')

parser.add_argument('--min-text-size', dest='min_text_size', action='store', default=10,
                    help='if the text size is smaller than this, we ignore it during training')
parser.add_argument('--min-crop-side-ratio', dest='min_crop_side_ratio', action='store', default=0.1, type=float,
                    help='if the text size is smaller than this, we ignore it during training')
...
def get_images():
    ...
    for ext in ['jpg', 'png', 'jpeg', 'JPG']:
        files.extend(glob.glob(
            os.path.join(FLAGS.training_images_path, '*.{}'.format(ext))))
    return files


def load_annotation_icdar(p):
    '''
...
load_annotation = load_annotation_icdar
...
def generator(input_size=512, batch_size=32,
    ...
    print('{} training images in {}'.format(
        image_list.shape[0], FLAGS.training_images_path))
    print('{} training labels in {}'.format(
        image_list.shape[0], FLAGS.training_labels_path))
    ...
    while True:
        ...
        for i in index:
            try:
                ...
                # print im_fn
                h, w, _ = im.shape
                txt_fn = os.path.join(FLAGS.training_labels_path, 'gt_' + os.path.basename(im_fn)).replace(
